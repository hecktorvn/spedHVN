...
use NFePHP\Common\DateTime;
use NFePHP\Common\LotNumber;
...
class Tools extends BaseTools
{
    ...
    public function sefazEnviaLote(
        ...
        //carrega serviço e verifica sua existencia
     ...
     * @internal function zLoadServico (Common\Base\BaseTools)
     */
    public function sefazConsultaChave($chave = '', $tpAmb = '')
    {
     ...
     * @throws Exception\InvalidArgumentException
     */
    public function sefazEncerra($chave, $dtEnc, $cUF, $cMun) {
        $uf = $this->validKeyByUF($chave);
        $xJust = Strings::replaceSpecialsChars(
            substr(trim($xJust), 0, 255)
        );
     ...
     * @return boolean
     */
    public function validarXml($version, $body, $method)
    {
        $xsdFile = $method.'_v'.$version.'.xsd';
        $xsdPath = __DIR__ . '/../schemes/PL_MDFe_' . preg_replace('/[^0-9]/', '', $version) . DIRECTORY_SEPARATOR . $xsdFile;
